...
CANAL_NOTIFICACIONES_ID = 1327809148666122343  # ID del canal de notificaciones

# Expresiones regulares para detectar enlaces válidos
...
async def on_message(message):
    ...
    if message.channel.id == CANAL_RESTRINGIDO_ID:
        ...
        # Si el mensaje NO contiene un enlace válido ni un archivo permitido
        if not (tiene_enlace_youtube or tiene_enlace_tiktok or tiene_mp4 or tiene_imagen):
            ...
            if canal_notificaciones:
                ...
                aviso = await canal_notificaciones.send(
                    f"⚠️ {message.author.name} intentó enviar contenido no permitido en el canal restringido: {message.content}\n"
        ...
        # Si el contenido es válido, mostrar advertencia leve
